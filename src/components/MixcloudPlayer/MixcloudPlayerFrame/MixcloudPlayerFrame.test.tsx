...
describe("MixcloudPlayerFrame", () => {
    it("renders a wrapping div with the given customClass", () => {
        render(<MixcloudPlayerFrame showTitle={MOCK_DATA[0].name} showUrl={MOCK_DATA[0].url} hideArtwork={false} customClass={"w-full"}/>);
    ...
    it("sets the iframe src to include the encoded showUrl", () => {
        ...
        render(<MixcloudPlayerFrame showTitle={MOCK_DATA[0].name} showUrl={showUrl} hideArtwork={hideArtwork} customClass={"w-full"}/>);
        const frame = screen.getByTestId("frame");
